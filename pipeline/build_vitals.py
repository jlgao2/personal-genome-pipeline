...
def write_vitals_js(parquet_dir: Path, out_path: Path,
    ...
    media = _load_media()
    correlations = _build_correlations(parquet_root)
    out_path.write_text(
        ...
        f"export const CORRELATIONS = {json.dumps(correlations, indent=2)};\n"
    )
...
def _build_correlations(parquet_root: Path) -> Optional[dict]:
    try:
        from pipeline.correlations import build_correlations
        return build_correlations(parquet_root)
    except Exception:
        return None
